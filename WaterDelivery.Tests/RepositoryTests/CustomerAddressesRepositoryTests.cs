...
public class CustomerAddressesRepositoryTests : BaseTest
{
    ...
    [Fact]
    public async Task Get_CustomerAddress_By_Id_Async_Should_Fail()
    {
        var test = async () =>
            await _customerAddressesRepository.GetCustomerAddressesByIdAsync(Guid.NewGuid(), CancellationToken.None);
    ...
    public async Task Delete_CustomerAddress_Should_Success()
    {
        ...
        (await _customerAddresses.Find(u => u.Id == resultId).FirstOrDefaultAsync(CancellationToken.None)).Should()
            .BeNull();
    }
